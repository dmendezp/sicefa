...
class ProgrammeController extends Controller
{
    ...
    public function management_search(Request $request)
    {
        ...
        // Obtener actividades extraordinarias para el ambiente seleccionado (solo cuando se filtra por ambiente)
        ...
        if ($option == 2) {
            ...
            if ($extraordinaryActivities->count() > 0) {
                foreach ($extraordinaryActivities as $activity) {
                    \Log::info("🎯 Actividad: {$activity->activity_name} - Fecha: {$activity->date} - Hora: {$activity->start_time} - {$activity->end_time}");
                }
            }
        }
        ...
        \Log::info("- Eventos de programación: " . $programmingEvents->count());
        \Log::info("- Actividades extraordinarias: " . $extraordinaryActivities->count());
        \Log::info("- Opción: $option");
        
        // Devolver las iniciales junto con la respuesta JSON
        return response()->json([
            'programmingEvents' => $programmingEvents,
            'extraordinaryActivities' => $extraordinaryActivities,
            'option' => $option,
            'initials' => $allInitials,
        ]);
    }
