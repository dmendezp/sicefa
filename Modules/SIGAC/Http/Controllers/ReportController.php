...
class ReportController extends Controller
{
    ...
    public function environments_search(Request $request){
        ...
        // Obtener ambientes ocupados por programación de instructores (todas las del día)
        ...
        // Obtener ambientes ocupados por actividades extraordinarias (todas las del día)
        $extraordinary_activities = EnvironmentActivityProgram::where('date', $day)
        ->pluck('environment_id')
        ->toArray();
        
        // Combinar todos los ambientes ocupados
        $occupied_environments = array_merge($instructor_program, $extraordinary_activities);
        $occupied_environments = array_unique($occupied_environments);
        
        $environments = Environment::pluck('id')->toArray();

        // Ambientes disponibles = todos los ambientes - ambientes ocupados
        $available_environment_ids = array_diff($environments, $occupied_environments);
        ...
        // Obtener información de programación de instructores para mostrar (todas las del día)
        $instructor_program_day = InstructorProgram::with('environment_instructor_programs.environment')
        ->where('date', $day)
        ->get();
        
        // Obtener información de actividades extraordinarias para mostrar (todas las del día)
        $extraordinary_activities_day = EnvironmentActivityProgram::with('environment')
        ->where('date', $day)
        ->get();
        ...
        return view('sigac::reports.environments.table_available')->with([
            ...
            'extraordinary_activities' => $extraordinary_activities_day
        ]);
    }
