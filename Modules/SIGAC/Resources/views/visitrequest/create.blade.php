    ...
    <div class="card">
        ...
        <div class="card-body">
            ...
            <div class="mb-3">
                ...
                <div class="btn-group">
                    ...
                    <label class="btn btn-outline-primary"
                        for="type_visita">{{ trans('sigac::visits.request.type.visit') }}</label>

                    <input type="radio" name="type" value="practica" id="type_practica" class="btn-check"
                        {{ old('type') === 'practica' ? 'checked' : '' }}>
                    <label class="btn btn-outline-primary"
                        for="type_practica">{{ trans('sigac::visits.request.type.practice') }}</label>
                </div>
            </div>
            ...
            <div id="req_wrapper" class="mb-3" style="display: none;">
                ...
                <small class="text-muted"> {{ trans('sigac::visits.practice.requirements.help') }}</small>
            </div>
            ...
            <div class="modal-dialog modal-lg modal-dialog-centered">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="modalTitle">{{ trans('sigac::general.Application registered') }}</h5>
                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                            aria-label="{{ trans('sigac::visits.modal.close') }}"></button>
                    </div>
                    <div class="modal-body">
                        <dl class="row">
                            <dt class="col-sm-4">{{ trans('sigac::visits.company.label') }}</dt>
                            <dd class="col-sm-8">{{ $vr['company'] }}</dd>

                            <dt class="col-sm-4">{{ trans('sigac::visits.contact.name') }}</dt>
                            <dd class="col-sm-8">{{ $vr['contact_name'] }}</dd>

                            <dt class="col-sm-4">{{ trans('sigac::visits.contact.email') }}</dt>
                            <dd class="col-sm-8">{{ $vr['contact_email'] }}</dd>

                            <dt class="col-sm-4">{{ trans('sigac::visits.modal.phone') }}</dt>
                            <dd class="col-sm-8">{{ $vr['contact_phone'] }}</dd>

                            <dt class="col-sm-4">{{ trans('sigac::visits.request.modal.type') }}</dt>
                            <dd class="col-sm-8 text-capitalize">{{ $vr['type'] }}</dd>

                            @if ($vr['type'] === 'practica')
                                <dt class="col-sm-4">{{ trans('sigac::visits.modal.requirements') }}</dt>
                                <dd class="col-sm-8">{{ $vr['requirements'] }}</dd>
                            @endif

                            <dt class="col-sm-4">{{ trans('sigac::visits.modal.state') }}</dt>
                            <dd class="col-sm-8"><span class="badge bg-info">{{ $vr['state'] }}</span></dd>

                            <dt class="col-sm-4">{{ trans('sigac::visits.modal.created_at') }}</dt>
