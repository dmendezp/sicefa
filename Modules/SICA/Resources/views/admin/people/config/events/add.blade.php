<div id="add-event">
    <div class="modal-header py-2">
        <h5 class="modal-title" id="exampleModalLabel"><b>Registrar evento</b></h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
            <span aria-hidden="true">&times;</span>
        </button>
    </div>
    {!! Form::open(['route'=>'sica.admin.people.config.event.add', 'method'=>'POST', 'id'=>'form-add-event']) !!}
        <div class="modal-body px-4 pt-0">
                @include('sica::admin.people.config.events.form')
        </div>
        <div class="modal-footer py-1">
                <button type="button" class="btn btn-secondary btn-md py-0" data-dismiss="modal">Cancelar</button>
                {!! Form::submit('Registrar', ['class'=>'btn btn-primary btn-md py-0']) !!}
        </div>
    {!! Form::close() !!}
</div>

{{-- Effect for status sending information --}}
<script>
    $('#form-add-event').submit(function () {
        $('#loader-message').text('Enviando información...'); /* Add content to loader */
        $("#add-event").hide(); /* Hide the content of the modal */
        $('#modal-content').append($('#modal-loader').clone()); /* Add the loader to the modal */
	});
</script>
