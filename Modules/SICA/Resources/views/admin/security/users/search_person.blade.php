        ...
        @foreach ($apps as $app)
            <div class="col-md-3 border border-1 p-3 bg-light">
                ...
                <div class="form-check">
                    <input class="form-check-input" name="roles_id[{{ $app->id }}]" type="radio" value="" id="no_role_{{ $app->id }}" {{ $user ? (!$user->roles->contains('app_id', $app->id) ? 'checked' : '') : 'checked' }}>
                    <label class="form-check-label" for="no_role_{{ $app->id }}"><em>Sin rol asignado</em></label>
                </div>
                @foreach ($app->roles->sortBy('name') as $role)
                    <div class="form-check">
                        <input class="form-check-input" name="roles_id[{{ $app->id }}]" type="radio" value="{{ $role->id }}" id="role_{{ $role->id }}" {{ $user ? ($user->roles->contains($role->id) ? 'checked' : '') : '' }}>
