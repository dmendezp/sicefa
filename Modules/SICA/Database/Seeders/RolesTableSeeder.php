...
class RolesTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        $user_admin = User::where('nickname','DiegoT')->first(); // Usuario Administrador (Diego Andrés Tovar Rodriguez)
        $user_coordinator = User::where('nickname','gmsanchez')->first(); // Usuario Administrador (Gloria Maritza Sanchez Alarcón)
        $user_attendance = User::where('nickname','JDGM0331')->first(); // Usuario Administrador (Jesús David Guevara Munar)
        ...
        $user_super_admin->roles()->syncWithoutDetaching([$rol_admin->id]);
