        ...
        var selectedElements = {}; // Objeto para mantener un registro de elementos seleccionados en cada fila
            ...
            function addtoolRow() {
                ...
                newRow.html(
                    ...
                    '<td class="col-2"><select id="tool-id" class="form-control tool-id" name="tool-id[]" required><option value="">Seleccione la Herramienta</option>@foreach ($toolOptions as $toolId => $toolName)<option value="{{ $toolId }}">{{ $toolName }}</option>@endforeach</select></td>' +
                    '<td class="col-2"><input type="number" id="quantitytool" class="form-control quantitytool" name="quantitytool[]" placeholder="Cantidad"><span class="quantity-message"></span></td>' +
                    '<td><input type="text" id="pricetool" class="form-control pricetool" name="pricetool[]" placeholder="Costo de uso"></td>' +
            ...
            $('#addtool').click(function() {
                ...
                if (lastRow.find('#tool-id').val() &&
                    lastRow.find('#quantitytool').val() && lastRow.find('#pricetool').val() &&
                    ...
                    var productNameSelect = newRow.find(
                        '.tool-id'); // Acceder a newRow en lugar de lastRow
            ...
            toolTable.on('change', 'select[name="tool-id[]"]', function() {
                ...
                $.ajax({
                    ...
                    error: function() {
                        var quantityField = currentRow.find('#quantitytool');
                        var quantityMessage = currentRow.find('.quantity-message');
            ...
            toolTable.on('input', 'input[name="pricetool[]"]', function() {
                var currentRow = $(this).closest('tr');
                var priceField = currentRow.find('#pricetool');
                var quantityField = currentRow.find('#quantitytool');
