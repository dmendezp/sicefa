    ...
    <div id="formagrochemical" style="display: none;">
        <div class="container" id="form">
            <!-- Agregar la tabla dinámica -->
            <div class="form-group">
                <h3 id="title">Agroquímico</h3>
                <table id="agrochemicalTable" class="table table-bordered">
                    <thead>
                        <tr>
                            ...
                            <th>Hectáreas</th>
                            <th>{{ trans('agrocefa::movements.1T_Amount') }} Lt-Kg</th>
        ...
        $(document).ready(function() {
            ...
            function addProductRow() {
                ...
                newRow.html(
                    ...
                    '<td class="col-2"><select id="product-id" class="form-control product-id" name="product-id[]" required><option value="">Seleccione el Agroquímico</option>@foreach ($agrochemicalOptions as $agrochemicalId => $agrochemicalName)<option value="{{ $agrochemicalId }}">{{ $agrochemicalName }}</option>@endforeach</select></td>' +
                    ...
                    '<td class="col-2"><input type="number" id="hectarea" class="form-control hectarea" name="hectarea[]" placeholder="Cantidad de Hectareas"><span class="hectarea"></span></td>' +
                    '<td class="col-2"><input type="number" id="product-quantity" class="form-control product-quantity" name="product-quantity[]" placeholder="Dósis X hectárea"><span class="quantity-message"></span></td>' +
            ...
            //INICIO funcion de convertir y mostrar la cantidad del elemento
            ...
            $.ajax({
                ...
                success: function(response) {
                    ...
                    var unitLabel = currentRow.find('#unit-label'); // Nuevo campo para mostrar la unidad de medida

                    
                    measurementUnitField.val(response.unidad_medida || 'Unidad de medida no encontrada');
                    measurementUnitField.val(unit);
                    unitLabel.text(unit);

                    var conversionFactor = 1;
                    if (unit === 'ml') {
                        conversionFactor = 0.001; // Convertir a litros si la unidad es ml
                    } else if (unit === 'gr') {
                        conversionFactor = 0.001; // Convertir a kilogramos si la unidad es gr
                        }
            ...
            //fin de funcion de convertir y mostrar la cantidad del elemento
            ...
            agrochemicalTable.on('change', 'select[name="product-id[]"]', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        if (amount !== null) {
                            quantityMessage.text(' Disponible: ' + amount ).removeClass(
                                'error-message');
                        } else {
