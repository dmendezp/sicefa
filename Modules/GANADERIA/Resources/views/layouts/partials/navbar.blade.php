      ...
      @guest
        ...
        @elseif(Auth::user()->roles[0]->slug=='ganaderia.veterinario')
          <li class="nav-item d-none d-sm-inline-block {{ ! Route::is('*veterinary.*') ?: 'active'}}">
            <a href="{{ route ('ganaderia.admin.veterinary.home') }}" class="nav-link">{{ trans('Veterinario')}}</a>
          </li>
        @elseif(Auth::user()->roles[0]->slug=='ganaderia.aprendiz_lider')
          <li class="nav-item d-none d-sm-inline-block {{ ! Route::is('*veterinary.*') ?: 'active'}}">
            <a href="{{ route ('ganaderia.admin.apprentice_leader.home2') }}" class="nav-link">{{ trans('aprendiz_lider')}}</a>
          </li>
        @endif
      @endguest
    </li>
