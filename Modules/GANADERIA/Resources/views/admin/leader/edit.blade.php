        ...
        <div class="col-lg-12">
          <div class="card card-success card-outline">
            <div class="card-header">
              <h3 class="m-0">{{ trans('ganaderia::menu.Edit') }} {{ trans('ganaderia::leader.Animal') }} {{ $editAnimal->name }}</h3>
            </div>
            <div class="card-body">
              <div class="content">
                <form action="{{ route('ganaderia.admin.leader.editpost') }}" method="post">
                  @csrf
                  ...
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" id="name" name="name" value="{{ $editAnimal->name }}">
                  </div>
                  <!-- fin del nombre -->
                  <!-- inicio del codigo -->
                  <div class="form-group">
                    <label for="code">{{ trans('ganaderia::leader.Code') }} {{ trans('ganaderia::leader.Of The') }} {{ trans('ganaderia::leader.Animal') }}</label>
                    <input type="text" class="form-control" id="code" name="code" value="{{ $editAnimal->code }}">
                  </div>
                  <!-- fin del codigo -->
                  <!-- inicio de la raza -->
                  <div class="form-group">
                    <label for="race">{{ trans('ganaderia::leader.Race') }} {{ trans('ganaderia::leader.Of The') }} {{ trans('ganaderia::leader.Animal') }}</label>
                    {!! Form::select('races_id',$raceedit, $editAnimal->races_id,['class' => 'form-control','placeholder' => 'Seleccione...','required']) !!}
                  </div>
                  <!-- fin de la raza -->
                  <!-- inicio de la madre -->
                  <div class="form-group">
                  ...
                  <div class="form-group">
                    ...
                    <input type="number" class="form-control" id="peso" name="peso" value="{{ $editAnimal->weight }}">
                  </div>
                  <!-- fin del peso -->
                  <!-- inicio del sexo -->
                  <div class="form-group">
                    <label for="sex">{{ trans('ganaderia::leader.Sex') }} {{ trans('ganaderia::leader.Of The') }} {{ trans('ganaderia::leader.Animal') }}</label>
                    {!! Form::select('sex',getEnumValues('animals','sex'),$editAnimal->sex,['class'=>'form-control','placeholder'=>'seleccione...']) !!}
                  </div>
                  <!-- fin del sexo -->
                  <!-- inicio del color -->
                  <div class="form-group">
                    <label for="color">{{ trans('ganaderia::leader.Color') }} {{ trans('ganaderia::leader.Of The') }} {{ trans('ganaderia::leader.Animal') }}</label>
                    <input type="text" class="form-control" id="color" name="color" value="{{ $editAnimal->color }}">
                  </div>
                  <!-- fin del color -->
                  <!-- inicio del fecha -->
                  <div class="form-group">
                    <label for="date">{{ trans('ganaderia::leader.Date') }} {{ trans('ganaderia::leader.Of') }} {{ trans('ganaderia::leader.Birth') }}</label>
                    <input type="date" class="form-control" id="date" name="date" value="{{ $editAnimal->date_of_birth }}">
                  </div>
                  <!-- fin del fecha -->
                  <!-- inicio del boton -->
