                    ...
                    <button type="button" class="btn btn-success" data-bs-toggle="modal" data-bs-target="#crearModal">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="#" class="btn btn-warning editar-btn" data-bs-toggle="modal" data-bs-target="#editarModal" data-id="{{ $contractor->id }}" data-nombre="{{ $contractor->name }}">Editar</a>
                                    <form action="{{ route('gth.contractortypes.delete', $contractor->id) }}" method="POST" id="btnEliminar" class="d-inline">
                                        @csrf
                                        @method('DELETE')
                                        <button  type="submit" class="btn btn-danger" data-id="{{ $contractor->id }}">Eliminar</button>
                                    </form>
                                </td>
                            </tr>
                            @endforeach
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                <form action="{{ route('gth.contractortypes.create') }}" method="POST">
                    @csrf
...
<div class="modal fade" id="editarModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @if(isset($contractor)) <!-- Cambiado a $contractor -->
                <form id="editForm" method="POST" action="{{ route('gth.contractortypes.update', ['id' => $contractor->id]) }}">
                    @csrf
                    @method('PATCH')
                    <input type="hidden" name="id" value="{{ $contractor->id }}"> <!-- Cambiado a $contractor -->
                    <div class="mb-3">
                        <label for="editName" class="form-label">Nombre</label>
                        <input type="text" class="form-control" id="editName" name="name" value="{{ old('name', $contractor->name) }}"> <!-- Cambiado a $contractor -->
                        @error('name')
...
@section('js')
    ...
    <script>
        ...
        document.getElementById("btnEliminar").addEventListener("click", function(event) {
            ...
            const contractorId = this.getAttribute("data-id"); // Obtiene el ID del empleado
...
<script>
    $(document).ready(function() {
        $('.editar-btn').click(function() {
            ...
            var updateRoute = '{{ route("gth.contractortypes.update", ["id" => ":id"]) }}'.replace(':id', id);
