...
@push('breadcrumbs')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ trans('sia::alliances.edit_title_page') }}</li>
@endpush

@section('content')
    <form action="{{ route('sia.alliances.update', $alliance->id) }}" method="POST" id="form-alliance">
        @csrf
        @method('PUT')
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0">{{ trans('sia::alliances.edit_title_view') }}</h4>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <div class="row g-3">
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="organization" id="organization" class="form-control @error('organization') is-invalid @enderror" required
                                value="{{ old('organization', $alliance->organization) }}">
                            @error('organization')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="email" name="email" id="email" class="form-control @error('email') is-invalid @enderror" required
                                value="{{ old('email', $alliance->email) }}">
                            @error('email')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $alliance->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" name="start_date" id="start_date" class="form-control @error('start_date') is-invalid @enderror" required
                                value="{{ old('start_date', $alliance->start_date) }}" min="{{ now()->format('Y-m-d') }}">
                            @error('start_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" name="end_date" id="end_date" class="form-control @error('end_date') is-invalid @enderror"
                                value="{{ old('end_date', $alliance->end_date) }}">
                            @error('end_date')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-select @error('status') is-invalid @enderror" required>
                                ...
                                <option value="active" {{ old('status', $alliance->status) == 'active' ? 'selected' : '' }}>
                                    {{ trans('sia::alliances.status_active') }}
                                </option>
                                <option value="inactive" {{ old('status', $alliance->status) == 'inactive' ? 'selected' : '' }}>
            ...
            <div class="card-footer bg-light text-end">
                ...
                <button type="submit" class="btn btn-primary btn-sm" id="btn-update-alliance">
                    <i class="fas fa-save"></i> {{ trans('sia::alliances.action_update') }}
                </button>
            </div>
        </div>
    </form>
    ...
    <script>
        $(document).ready(function() {
            $('#form-alliance').on('submit', function(e) {
                $('#btn-update-alliance').prop('disabled', true).html(
