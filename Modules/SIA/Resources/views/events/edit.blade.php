...
@push('breadcrumbs')
    ...
    <li class="breadcrumb-item active">{{ trans('sia::eventsia.edit_title_page') }}</li>
@endpush

@section('content')
    <form action="{{ route('sia.admin.events.update', $event->id) }}" method="POST" id="form-event" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card card-success card-outline mx-auto mb-3 custom-border-color">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="event_image" id="event_image" class="form-control" value="{{ $event->event_image }}" required>
                            @error('event_image')
                                <div class="alert alert-danger py-0 my-1">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="date" name="start_date" id="start_date" class="form-control" value="{{ $event->start_date ? $event->start_date->format('Y-m-d') : '' }}" required>
                            @error('start_date')
                                <div class="alert alert-danger py-0 my-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="date" name="end_date" id="end_date" class="form-control" value="{{ $event->end_date ? $event->end_date->format('Y-m-d') : '' }}" required>
                            @error('end_date')
                                <div class="alert alert-danger py-0 my-1">{{ $message }}</div>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="email" name="contact_email" id="contact_email" class="form-control" value="{{ $event->contact_email }}" required>
                            @error('contact_email')
                                <div class="alert alert-danger py-0 my-1">{{ $message }}</div>
                            @enderror
                        </div>
                        <div class="form-group">
                            ...
                            <input type="number" name="contact_phone" id="contact_phone" class="form-control" value="{{ $event->contact_phone }}">
                            @error('contact_phone')
                                <div class="alert alert-danger py-0 my-1">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-12 mt-4">
                        ...
                        <div class="form-group">
                            ...
                            <select name="status" id="status" class="form-control" required>
                                ...
                                <option value="scheduled" {{ $event->status == 'scheduled' ? 'selected' : '' }}>
                                    {{ trans('sia::eventsia.status_scheduled') }}
                                </option>
                                <option value="ongoing" {{ $event->status == 'ongoing' ? 'selected' : '' }}>
                                    {{ trans('sia::eventsia.status_ongoing') }}
                                </option>
                                <option value="completed" {{ $event->status == 'completed' ? 'selected' : '' }}>
                                    {{ trans('sia::eventsia.status_completed') }}
                                </option>
                                <option value="cancelled" {{ $event->status == 'cancelled' ? 'selected' : '' }}>
                                    {{ trans('sia::eventsia.status_cancelled') }}
                                </option>
                            </select>
                            @error('status')
                                <div class="alert alert-danger py-0 my-1">{{ $message }}</div>
            ...
            <div class="card-footer bg-white text-right">
                ...
                <button type="submit" class="btn btn-sm btn-success" id="btn-update-event">
                    <b>{{ trans('sia::eventsia.action_update') }}</b>
                </button>
            </div>
        </div>
    </form>
    ...
    <script>
        $(document).ready(function() {
            $("#form-event").submit(function() {
                ...
                toastr.success('{{ trans("sia::eventsia.updating") }}', '{{ trans("sia::eventsia.processing") }}');
            });
        });
    </script>
@endpush