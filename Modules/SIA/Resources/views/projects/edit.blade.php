...
@push('breadcrumbs')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ trans('sia::projects.edit_title_page') }}</li>
@endpush

@section('content')
    <form action="{{ route('sia.admin.projects.update', $project->id) }}" method="POST" id="form-project" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0">{{ trans('sia::projects.edit_title_view') }}</h4>
            </div>
            <div class="card-body">
                @if ($errors->any())
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="form-group">
                            ...
                            <input type="file" name="pdf_report_path" id="pdf_report_path" class="form-control @error('pdf_report_path') is-invalid @enderror"
                                accept=".pdf">
                            <small class="text-muted">{{ $project->pdf_report_path ? basename($project->pdf_report_path) : 'Sin archivo' }}</small>
                        ...
                        <div class="form-group">
                            ...
                            <select name="estado" id="estado" class="form-select @error('estado') is-invalid @enderror" required>
                                ...
                                <option value="EN_CURSO" {{ old('estado', $project->estado) == 'EN_CURSO' ? 'selected' : '' }}>
                                    {{ trans('sia::projects.estado_EN_CURSO') }}
                                </option>
                                <option value="FINALIZADO" {{ old('estado', $project->estado) == 'FINALIZADO' ? 'selected' : '' }}>
                                    {{ trans('sia::projects.estado_FINALIZADO') }}
                                </option>
                                <option value="CANCELADO" {{ old('estado', $project->estado) == 'CANCELADO' ? 'selected' : '' }}>
            ...
            <div class="card-footer bg-light text-end">
                ...
                <button type="submit" class="btn btn-primary btn-sm" id="btn-update-project">
                    <i class="fas fa-save"></i> {{ trans('sia::projects.action_update') }}
                </button>
            </div>
        </div>
    </form>
    ...
    <script>
        $(document).ready(function() {
            $('#form-project').on('submit', function(e) {
                $('#btn-update-project').prop('disabled', true).html(
