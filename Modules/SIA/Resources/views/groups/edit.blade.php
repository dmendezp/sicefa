...
@push('breadcrumbs')
    ...
    <li class="breadcrumb-item active" aria-current="page">{{ trans('sia::groups.edit_title_page') }}</li>
@endpush

@section('content')
    <form action="{{ route('sia.groups.update', $group->id) }}" method="POST" id="form-group">
        @csrf
        @method('PUT')
        <div class="card shadow-sm border-0 mb-4">
            <div class="card-header bg-primary text-white">
                <h4 class="mb-0">{{ trans('sia::groups.edit_title_view') }}</h4>
            </div>
            <div class="card-body">
                @if ($errors->any())
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <textarea name="description" id="description" class="form-control @error('description') is-invalid @enderror" required>{{ old('description', $group->description) }}</textarea>
                            @error('description')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
            ...
            <div class="card-footer bg-light text-end">
                ...
                <button type="submit" class="btn btn-primary btn-sm" id="btn-update-group">
                    <i class="fas fa-save"></i> {{ trans('sia::groups.action_update') }}
                </button>
            </div>
        </div>
    </form>
    ...
    <script>
        $(document).ready(function() {
            $('#form-group').on('submit', function(e) {
                $('#btn-update-group').prop('disabled', true).html(
