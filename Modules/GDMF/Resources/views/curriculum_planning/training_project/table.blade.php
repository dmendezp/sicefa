        ...
        <table id="training_project" class="display table table-bordered table-striped table-sm">
            <thead>
                <tr>
                    ...
                    <th class="text-center">Asignar Presupuesto</th>
            ...
            <tbody>
                @foreach ($coursesWithTrainingProjects as $course)
                    @foreach ($course->training_projects as $t)
                        @php
                            $budget = $projectBudgets[$t->id] ?? null;
                        @endphp
                        <tr>
                            ...
                            <td class="text-center">{{ $counts[$t->id] ?? 0 }} de
                                {{ $course->program->competencies->flatMap(function ($competency) {
                                        return $competency->learning_outcomes;
                                    })->count() }}
                            </td>
                            <td class="text-center">
                                <a class="btn btn-warning" data-bs-toggle="modal"
                                    data-bs-target="#informationproyecto{{ $t->id }}">
                                    <i class="fas fa-eye"></i>
                                </a>
                            </td>
                            <td class="text-center">
                                <a class="btn {{ $budget ? 'btn-primary' : 'btn-success' }}" data-bs-toggle="modal"
                                    data-bs-target="#crearpresupuesto{{ $t->id }}">
                                    <i class="fas fa-dollar-sign"></i>
                                </a>
                            </td>
                            @include('gdmf::curriculum_planning.training_project.budget', ['budgetData' => $budget])
                            <td class="text-center col-1">
                                <div class="opts">
                                    <a data-bs-toggle="modal" data-bs-target="#editproyecto{{ $t->id }}">
                                        <b class="text-primary" data-toggle="tooltip" data-placement="top"
                                            title="Actualizar Proyecto Formativo">
                                            <i class="fas fa-edit"></i>
                                        </b>
                                    </a>
                                    @include('gdmf::curriculum_planning.training_project.information')
                                    @include('gdmf::curriculum_planning.training_project.edit')
                                    <a class="delete-training_project" data-trainingproject-id="{{ $t->id }}">
                                        <b class="text-danger" data-toggle="tooltip" data-placement="top"
                            ...
                            <form id="delete-trainingproject-form-{{ $t->id }}"
                                action="{{ route('gdmf.academic_coordination.curriculum_planning.training_project.destroy', ['id' => $t->id]) }}"
                                method="POST">
                                @csrf
                                @method('DELETE')
                            </form>
                    ...
                    @endforeach
                @endforeach
                @include('gdmf::curriculum_planning.training_project.create')
            </tbody>
        </table>
    </div>
</div>
        ...
        $('.delete-training_project').on('click', function(event) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    ...
                    document.getElementById('delete-trainingproject-form-' + trainingproject_id)
                        .submit();
                }
            });
        });
