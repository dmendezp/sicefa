...
<div class="glass-container">
    ...
    <form action="{{ route('cefa.gdf.store_certificate') }}" method="POST" id="cert-form">
        @csrf

        <div class="form-group">
            <label for="codigo_certificado">Código del Certificado</label>
            <input type="text" id="codigo_certificado" name="certified_code" maxlength="10" placeholder="Solo números" required>
        </div>

        <div class="form-group">
            <label for="fecha_emision">Fecha de Emisión</label>
            <input type="date" id="fecha_emision" name="issue_date" required>
        </div>

        <div class="form-group">
            <label for="cedula_funcionario">Cédula del Funcionario</label>
            <input type="text" id="cedula_funcionario" name="official_id" maxlength="10" placeholder="Solo números" required>
        </div>

        <div class="form-group">
            <label for="descripcion">Descripción</label>
            <textarea id="descripcion" name="description" rows="4" placeholder="Describe el certificado..." required></textarea>
        </div>

        <button type="submit" class="btn-submit"><i class="fas fa-save"></i> Guardar Certificado</button>
    </form>
</div>
