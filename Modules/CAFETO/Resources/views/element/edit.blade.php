...
@push('breadcrumbs')
    <li class="breadcrumb-item active">
        <a href="{{ route('cafeto.element.index') }}" class="text-decoration-none">Productos</a>
    </li>
    <li class="breadcrumb-item active">Actualizar producto</li>
@endpush

@section('content')
    <form action="{{ route('cafeto.element.update', $element) }}" id="form-element" method="POST" enctype="multipart/form-data">
        @csrf
        <div class="card card-success card-outline mx-auto">
            <div class="card-body">
                <div class="row">
                    <div class="col-md-4">
                        <div class="card card-success border-success">
                            ...
                            <div class="card-body text-center">
                                <img
                                    src="@if($element->image && file_exists(public_path($element->image))) {{ asset($element->image) }} @else {{ asset('modules/sica/images/sinImagen.png') }} @endif"
                    ...
                    <div class="col-md-4">
                        {!! Form::label('name', 'Nombre del elemento:', ['class' => 'mt-3']) !!}
                        ...
                        <div class="input-group">
                            ...
                            <select name="measurement_unit_id" class="form-select" required>
                                ...
                                @foreach ($measurement_units as $mu)
                                    <option value="{{ $mu->id }}" {{ $element->measurement_unit_id == $mu->id ? 'selected' : '' }}>{{ $mu->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div class="col-md-4">
                        ...
                        <div class="input-group">
                            ...
                            <select name="category_id" class="form-select" required>
                                ...
                                @foreach ($categories as $c)
                                    <option value="{{ $c->id }}" {{ $element->category_id == $c->id ? 'selected' : '' }}>{{ $c->name }}</option>
                                @endforeach
                            </select>
                        </div>
                        @error('category_id')
                        ...
                        {!! Form::label('UNSPSC_code', 'Codigo UNSPSC', ['class' => 'mt-3']) !!}
                        <div class="input-group">
                            <div class="input-group-prepend">
                                <span class="input-group-text">
                                    <i class="far fa-keyboard"></i>
                        ...
                        <div class="input-group">
                            ...
                            <select name="kind_of_purchase_id" class="form-select" required>
                                ...
                                @foreach ($kind_of_purchases as $kp)
                                    <option value="{{ $kp->id }}" {{ $element->kind_of_purchase_id == $kp->id ? 'selected' : '' }}>{{ $kp->name }}</option>
                                @endforeach
                            </select>
                        </div>
