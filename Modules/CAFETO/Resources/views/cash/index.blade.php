            ...
            <div class="card" data-aos="zoom-in-down">
                <div class="card-body">
                    <div class="d-flex justify-content-center align-items-center flex-column">
                        <h4 class="text-center">{{ trans('cafeto::cash.Title_Card_Cash_Closing') }}</h4>
                        <div class="mt-3">
                            @if (!isset($active_cash))
                                @if (Auth::user()->havePermission('cafeto.' . getRoleRouteName(Route::currentRouteName()) . '.cash.store'))
                                    {!! Form::open([
                                        'route' => 'cafeto.' . getRoleRouteName(Route::currentRouteName()) . '.cash.store',
                                        'class' => 'form-row',
                                    ]) !!}
                                    <button type="submit" class="btn btn-success btn-block w-auto">
                                        <i class="fas fa-check"></i> {{ trans('cafeto::cash.Btn_Open_Cash') }}
                                    </button>
                                    {!! Form::close() !!}
                                @endif
                            @endif
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped table-hover">
                            <thead class="table-dark">
                                <tr>
                                    <th scope="col">{{ trans('cafeto::cash.1T_Number') }} </th>
                                    <th scope="col">{{ trans('cafeto::cash.1T_Opening_Manager') }} </th>
                                    <th scope="col">{{ trans('cafeto::cash.1T_Opening_Date') }}</th>
                                    <th scope="col">{{ trans('cafeto::cash.1T_Initial_Balance') }}</th>
                                    <th scope="col">{{ trans('cafeto::cash.1T_Final_Balance') }}</th>
                                    <th scope="col">{{ trans('cafeto::cash.1T_State') }}</th>
                                    <th scope="col"><i class="fa-solid fa-sort"></i>
                                        {{ trans('cafeto::cash.1T_Action') }}
                                    </th>
                                </tr>
                            </thead>
                            <tbody>
                                        ...
                                        <td>
                                            @if (Auth::user()->havePermission('cafeto.' . getRoleRouteName(Route::currentRouteName()) . '.cash.close'))
                                                <div data-bs-toggle="tooltip" data-bs-placement="bottom"
                                                    data-bs-title="{{ trans('cafeto::cash.Text_Tooltip_Closed') }}">
    ...
    <div class="row">
        <div class="col-md-12">
            <div class="card" data-aos="zoom-in-up">
                <div class="card-body">
                    <h4 class="text-center">{{ trans('cafeto::cash.Title_Card_Cash_History') }}</h4>
                    <hr>
                    <div class="table-responsive">
                        <table class="table table-striped table-hover" id="tableCashCountAll">
                            <thead class="table-dark">
                                <tr>
                                    <th scope="col" class="text-center">{{ trans('cafeto::cash.2T_Number') }}</th>
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h1 class="modal-title fs-5" id="exampleModalLabel">
                        {{ trans('cafeto::cash.Title_Modal') }}
                    </h1>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>
                <div class="modal-body">
                    {!! Form::open([
                        'route' => 'cafeto.' . getRoleRouteName(Route::currentRouteName()) . '.cash.close',
                        'id' => 'cierre-caja-form',
                        'class' => 'form-row',
                    ]) !!}
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            form.addEventListener('submit', (event) => {
                ...
                Swal.fire({
                    title: '{{ trans('cafeto::cash.TitleClosingCash') }}',
                    text: '{{ trans('cafeto::cash.TextClosingCash') }}',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: '{{ trans('cafeto::cash.Btn_Close_Cash') }}',
                    cancelButtonText: '{{ trans('cafeto::cash.Btn_Cancel') }}'
                }).then((result) => {
                    if (result.isConfirmed) {
                        form.submit();
                    } else {
                        Swal.fire(
                            '{{ trans('cafeto::cash.TitleClosingCashCancel') }}',
                            '{{ trans('cafeto::cash.TextClosingCashCancel') }}',
                            'info'
                        );
                    }
                });
            ...
            if (successMessage) {
                Swal.fire('{{ trans('cafeto::cash.TitleSuccess') }}', successMessage, 'success');
            }

            if (errorMessage) {
                Swal.fire('{{ trans('cafeto::cash.TitleAlert') }}', errorMessage, 'error');
            }
        });
    </script>
    <script src="{{ asset('modules/cafeto/js/cash/dateTimeNow.js') }}"></script>
@endpush
